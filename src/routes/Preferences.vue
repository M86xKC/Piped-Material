<template>
	<v-container style="max-width: 1080px">
		...
		<v-text-field v-if="$store.getters['prefs/getPreferenceBoolean']('enableCustomInstance', false)"
			...
			:value="$store.getters['prefs/getPreference']('instance')" @input="setValue('instance', $event)" />
...
export default {
	...
	methods: {
		...
		async getInstances () {
			...
			if (currentInstance && !instances.includes(currentInstance)) {
		...
		setCustomInstanceStatus (ev) {
			if (ev === false) {
				window.alert(this.$t('preferences.custom_instance.disabled_alert'))
			}
